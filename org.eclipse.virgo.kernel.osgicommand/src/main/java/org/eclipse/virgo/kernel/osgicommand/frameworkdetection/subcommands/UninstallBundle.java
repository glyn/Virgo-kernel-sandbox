...
/**
 * Handles the uninstall of a bundle in a target framework
 */
public class UninstallBundle implements SubCommandExecutor {
    ...
    public String execute(long frkIndex, CommandInterpreter interpreter) throws Exception {
        ...
        try {
            ...
            if (bnd != null) {
                FrameworkInfoUtils.uninstall(bnd);
                output.append("Bundle [").append(FrameworkInfoUtils.getSymbolicName(bnd)).append("_").append(FrameworkInfoUtils.getVersion(bnd)).append("] uninstalled successfully in framework [").append(frkIndex).append("]!").append(NEW_LINE);
            } else {
