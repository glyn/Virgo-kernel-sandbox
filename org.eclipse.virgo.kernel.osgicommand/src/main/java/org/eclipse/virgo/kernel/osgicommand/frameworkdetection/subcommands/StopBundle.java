...
/**
 * Handles the stop operations of bundles in a target framework
 */
public class StopBundle implements SubCommandExecutor {
    ...
    public String execute(long frkIndex, CommandInterpreter interpreter) throws Exception {
        ...
        try {
            ...
            if (bnd != null) {
                if (FrameworkInfoUtils.getState(bnd) != Bundle.RESOLVED && FrameworkInfoUtils.getState(bnd) != Bundle.INSTALLED) {
                    FrameworkInfoUtils.stop(bnd);
                    output.append("Bundle [").append(FrameworkInfoUtils.getSymbolicName(bnd)).append("_").append(FrameworkInfoUtils.getVersion(bnd)).append("] stopped successfully in framework [").append(frkIndex).append("]!").append(NEW_LINE);
                } else {
                    output.append("Bundle [").append(FrameworkInfoUtils.getSymbolicName(bnd)).append("_").append(FrameworkInfoUtils.getVersion(bnd)).append("] already stopped in framework [").append(frkIndex).append("]!").append(NEW_LINE);
                }
            } else {
