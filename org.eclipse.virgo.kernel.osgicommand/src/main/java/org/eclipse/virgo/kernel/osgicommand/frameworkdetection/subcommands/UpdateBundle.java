...
/**
 * Handles the update operations of bundles in a target framework
 */
public class UpdateBundle implements SubCommandExecutor {
    ...
    public String execute(long frkIndex, CommandInterpreter interpreter) throws Exception {
        ...
        try {
            ...
            if (bnd != null) {
                FrameworkInfoUtils.update(bnd);
                output.append("Bundle [").append(FrameworkInfoUtils.getSymbolicName(bnd)).append("_").append(FrameworkInfoUtils.getVersion(bnd)).append("] updated successfully in framework [").append(frkIndex).append("]!").append(NEW_LINE);
            } else {
